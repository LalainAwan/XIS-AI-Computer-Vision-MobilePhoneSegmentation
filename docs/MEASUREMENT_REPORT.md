...
The predicted segmentation mask generated by the trained Mask R-CNN model was converted into a binary mask using a threshold of 0.5.

The largest contour corresponding to the mobile phone was extracted using OpenCV contour detection.

To obtain object dimensions independent of image rotation, a minimum-area rotated bounding rectangle was fitted to the contour using OpenCV's cv2.minAreaRect() function.

The resulting rectangle provided:

Long side (pixels)
Short side (pixels)

These dimensions were used for pixel-to-millimetre conversion.

The physical dimensions of the phone were measured manually:
...
The conversion factor was computed as:

mm_per_pixel = Physical Dimension / Pixel Dimension

This conversion enables transformation of image-space measurements into real-world units.

---

6. Dimension Estimation

The long and short sides extracted from the segmentation mask were converted into millimetres using the computed pixel-to-mm conversion factors.
...
The experiment demonstrates the complete measurement pipeline:
...
The estimated dimensions match the known dimensions because the conversion factor was derived using the measured object dimensions. Therefore, this section validates the measurement methodology rather than serving as an independent measurement-accuracy benchmark.
---

## 7. Measurement Implementation

The following OpenCV operations were used:

cv2.findContours()
cv2.minAreaRect()

The contour extraction stage identifies the segmented phone boundary, while the rotated bounding rectangle provides dimensions that remain consistent even when the phone is rotated within the image.

Using a rotated rectangle reduces orientation-related measurement errors compared with a standard axis-aligned bounding box.
---

## 8. Segmentation Performance
